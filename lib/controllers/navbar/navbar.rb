module NavbarController
	...
	def get_value_class(user)
		count = SessionController.get(session[:user_uuid]).r_messages.all(new_message: true).count
		if count < 9
	...
	def needs_notification?(user)
		SessionController.get(session[:user_uuid]).r_messages.all(new_message: true).count > 0
	end
end